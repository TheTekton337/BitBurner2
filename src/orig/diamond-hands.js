import { getServerUpgradeMinCost } from "./aps-utils";
...
export async function main(ns) {
	...
	async function yolo(stocks) {
		const minServerUpgradeCost = getServerUpgradeMinCost(ns);
		let riskThresh = 20 * fees;
	
		const playerMoney = ns.getPlayer().money;

		ns.print("Player money: " + ns.nFormat(playerMoney, '$0.00a'));

		ns.print("Min upgrade cost: " + ns.nFormat(minServerUpgradeCost, '$0.00a'));
		let moneyLimitMultiplier = 0.25;
		if (hasInitialized && minServerUpgradeCost && playerMoney < minServerUpgradeCost) {
			// Player can't afford upgrade
			riskThresh = riskThresh + minServerUpgradeCost;
		}
    let money = ns.getPlayer().money * moneyLimitMultiplier;
    if (money < riskThresh) {
      ns.print("Risk thresh: " + ns.nFormat(riskThresh, '$0.00a'));
      ns.print("Money: " + ns.nFormat(money, '$0.00a'));
      const diff = riskThresh - money;
      ns.print("Diff: " + ns.nFormat(diff, '$0.00a'));
      ns.print("-------------------");
    }
		for (const stock of stocks) {
			money = ns.getPlayer().money * moneyLimitMultiplier; //Limit the money used to 25% of player funds
