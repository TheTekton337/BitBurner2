...
export async function attemptAllContracts(ns, isDebug = false) {
    const contracts = getContracts(ns);
    ns.print(`Found ${contracts.length} contracts.`);
    for (const contract of contracts) {
        await attemptContract(ns, contract, isDebug);
    }
}
...
export async function attemptContract(ns, contract, isDebug = false) {
    ...
    if (solver) {
        ...
        try {
            ...
            if (reward) {
                isDebug && ns.tprint(`${reward} for solving "${contract.type}" on ${contract.host}`);
                ns.print(`${reward} for solving "${contract.type}" on ${contract.host}`);
            }
            else {
                isDebug && ns.tprint(`ERROR: Failed to solve "${contract.type}" on ${contract.host}`);
